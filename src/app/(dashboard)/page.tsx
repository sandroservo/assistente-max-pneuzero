...
function getStatusBadge(status: string, ownerType: string) {
  ...
  const statusMap: Record<string, { label: string; className: string }> = {
    NOVO: { label: "Novo", className: "bg-red-100 text-red-700" },
...
export default async function DashboardPage() {
  ...
  const statusLabels: Record<string, { label: string; color: string }> = {
    NOVO: { label: "Novo", color: "#EF4444" },
    EM_ATENDIMENTO: { label: "Atendimento", color: "#F97316" },
      ...
      <div className="grid gap-4 md:gap-6 grid-cols-1 lg:grid-cols-3">
        <div className="lg:col-span-2 bg-white rounded-xl p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link
              href="/chats"
              className="px-4 py-2 bg-[#CC0000] text-white text-sm font-medium rounded-lg hover:bg-[#990000] transition-colors"
            >
              Ver conversas
            </Link>
          </div>
              ...
              conversationsInAttendance.map((conv) => {
                ...
                return (
                  ...
                  >
                    ...
                    {conv.unreadCount > 0 && (
                      <span className="shrink-0 w-5 h-5 rounded-full bg-[#CC0000] text-white text-xs flex items-center justify-center font-medium">
